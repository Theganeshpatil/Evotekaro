...
def create(request: schemas.Election, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_admin_user)):
...
def update(id: int, request: schemas.Election, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_admin_user)):
